...
| Q-12 | major | 1 | `confirm` drops a pair when one side fails ("a failed run costs its pair, not the comparison"). When the failures are the *candidate's* -- in the validation of `pyvrp-hard-1` both tuned configurations ran into the 900 s timeout on `t02`, seed 101, twice each, printing nothing, while the defaults finished -- dropping the pair removes exactly the runs where the candidate did worst, and the mean improvement is flattered. The count is printed ("N run(s) failed") but it is not part of the verdict or of the exit code. A candidate that does not answer is not "not comparable", it lost. | open |
